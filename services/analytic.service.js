...
const { outlet, pelanggan, transaksi, user } = require('../models');

const day = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jum\'at', 'Sabtu'];

async function graphService(requestToken) {
...
async function totalDataService(requestToken) {
  const token = jwt.decode(requestToken);
...
async function incomeService(requestToken, query) {
  const token = jwt.decode(requestToken);
...
function dateTracker(rawDate, fromNow) {
