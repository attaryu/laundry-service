...
const { createClientService } = require('./client.service');

async function createTransactionService({ requestToken, body }) {
  // identify role
  ...
  try {
    // check outlet
    const getOutlet = await user.findFirst({
      where: {
        id: token.id,
      },
      select: {
...
async function cancelTransactionService({ requestToken, params }) {
  // identify role
  const token = jwt.decode(requestToken);
...
async function paidOffTransactionService({ requestToken, params }) {
  // identify role
  const token = jwt.decode(requestToken);
...
async function changeStatusTransactionService({ requestToken, params, query }) {
  // identify role
  const token = jwt.decode(requestToken);
...
async function getAllTransactionService(query) {
  let filter = {};
...
async function getSpecificTransactionService(params) {
