...
const { logSurat, surat } = require('../models');

async function createEmailService({ requestToken, body }) {
...
async function getAllEmailService({ requestToken, query }) { 
  const token = jwt.decode(requestToken);
  let filter = {
    OR: [
      {
...
async function getSpecificEmailService({requestToken, params}) {
  const token = jwt.decode(requestToken);
...
async function deleteEmailService({ requestToken, body }) {
  const token = jwt.decode(requestToken);
...
async function archiveEmailService({ requestToken, params }) {
  const token = jwt.decode(requestToken);
...
async function readEmailService({ requestToken, params }) {
  const token = jwt.decode(requestToken);
