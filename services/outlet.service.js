...
const { logOutlet, outlet, transaksi, user } = require('../models');

async function createOutletService(requestToken, body) {
...
async function getAllOutletService(query) {
  let filter = {};
...
async function getSpecificOutletService(params) {
  try {
...
async function editOutletService(requestToken, body, params) {
  const token = jwt.decode(requestToken);
...
async function deleteOutletService({ requestToken, params }) {
  const token = jwt.decode(requestToken);
...
async function getNameOutletService() {
  try {
    const payload = await outlet.findMany({
      select: {
        id: true,
