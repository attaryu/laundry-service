...
const { logPelanggan, pelanggan, transaksi } = require('../models');

async function createClientService(requestToken, body) {
...
async function getAllClientService(query) {
  const filter = {
    AND: [],
  };
...
async function getSpecificClientService(params) {
  try {
...
async function editClientService(requestToken, body, params) {
  const token = jwt.decode(requestToken);
...
async function deleteClientService({ requestToken, params }) {
  const token = jwt.decode(requestToken);
  ...
  try {
    const existingClient = await pelanggan.findUnique({
      where: {
        id: params.customerId,
      },
      select: {
...
async function getNameClientService() {
  try {
